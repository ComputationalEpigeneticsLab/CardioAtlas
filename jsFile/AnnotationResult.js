...
if(UserTaskID=="functionExample"){webpath='example/functionExample';}
...
//// 1 table infor
document.getElementById("taskID").innerHTML=summary_re.UserTaskID;
document.getElementById("fileName").innerHTML=summary_re.fileName;
document.getElementById("ref").innerHTML=summary_re.share_reference;
document.getElementById("resolution").innerHTML=summary_re.resolution;
document.getElementById("geneNum").innerHTML=summary_re.gene_num;
document.getElementById("cellNum").innerHTML=summary_re.cell_num;
document.getElementById("nFeature").innerHTML=advance_re.nFeature_RNA_form+" ~ "+advance_re.nFeature_RNA_to;
document.getElementById("nCount").innerHTML=advance_re.nCount_RNAt_form+" ~ "+advance_re.nCount_RNA_to;
document.getElementById("mt").innerHTML=advance_re.percent_mt_form+"% ~ "+advance_re.percent_mt_to+"%";
document.getElementById("celltypeNum").innerHTML=advance_re.celltype_unique.split(",").length;
...
cluster_num=Number(cluster_re.cluster_num);
	...
	tsen_umap_org(data_umap_cluster,shapes_umap_cluster,"umap_cluster","umap");
	tsen_umap_org(data_tsen_cluster,shapes_tsen_cluster,"tsne_cluster","tsne");
/////3. marker
	var oldTable = $('#marker_table').dataTable();
	oldTable.fnClearTable(); //清空一下table
	oldTable.fnDestroy(); //还原初始化了的dataTable
	marker_table(webpath+"/Celltype_table.txt",'marker_table');
	var downloadPath="download_loading.jsp?path="+webpath+"/Celltype_table_Download.txt"+"&name=marker_download.txt";
	document.getElementById('marker_table_download').href = downloadPath;
	////
	document.getElementById("marker_heatmap_img").src = webpath+"/Celltype_heatmap.png";
	//violin
	// violin style
	...
	//xmslect celltype
			...
			var demo1_gene = xmSelect.render({
				...
				on: function(data){//监听
					...
					//
					var celltype_marker=advance_re[arr[0]["value"]+"_marker"].split(",");
					//1.marker
					celltypeMarkerXMselect_2(celltype_marker,style_all_violin,webpath);
		...
		var celltype_marker_first=advance_re[celltype_first+"_marker"].split(",");
		//1.marker
		celltypeMarkerXMselect_2(celltype_marker_first,style_all_violin,webpath);
...
///////////5. cell cell comm
		cellcell_communication_table(webpath+"/user_iTALK_method_top_genes_r_re.txt","tableCellCellCom");
		// 向download table中传参
		var downloadPath="download_loading.jsp?path="+webpath+"/user_iTALK_method_top_genes_r_re.txt&name=cellcellCommunication.txt";
		document.getElementById('tableCellCellCom_download').href = downloadPath;
		// river
		echars_River_cellcell(webpath+"/user_iTALK_method_top_genes_r_re_riverplot.json",'RiverCellCellCom');
////////////////////////////////////////////////////////////////////method///////////////////////////////////////////////////////////////
...
function celltypeMarkerXMselect(celltype_marker,style_all_violin,ats,webpath,annotationType){
	var data_option=celltype_marker;
		...
		for (var i=0;i<data_option.length;i++)
					...
					var demo1_gene = xmSelect.render({
						...
						on: function(data){//监听
							...
							CopyViolinFileAndPlot(ats+"@"+annotationType,webpath,style_all_violin,marker0);
		...
		//violin
		CopyViolinFileAndPlot(ats+"@"+annotationType,webpath,style_all_violin,data_option[0]);
		// feature plot////////////
...
function tsen_umap_org(data,shapes,div,type){
	if(type.match(RegExp(/tsne/))){t0="tSNE"}else{t0="UMAP"}
	var width0=$("#overview").width()*0.48;
	var data=data;
	var div=div;
	var layout = {
	       // shapes: shapes,
...
function celltypeMarkerXMselect_2(celltype_marker,style_all_violin,webpath){
	var data_option=celltype_marker;
		var data_option_valuearray = [];
		for (var i=0;i<data_option.length;i++)
					{ 
					...
					var e="#sc_celltype_marker_select";
					var demo1_gene = xmSelect.render({
						el: e, 
						size: 'big',//选中的之后显示的字的大小
					    theme: {color: '#f0ad4e',},//主题颜色
						...
						on: function(data){//监听
							...
							var isAdd = data.isAdd;
							var marker0=arr[0]["value"];
							var filepath0=webpath+"/violin/"+marker0+"_violin.csv";
							gene_infor_plotly_violin(filepath0,marker0,style_all_violin,"markerViolin");
		...
		//violin
		var filepath0=webpath+"/violin/"+data_option[0]+"_violin.csv";
		gene_infor_plotly_violin(filepath0,data_option[0],style_all_violin,"markerViolin");
		// feature plot////////////
