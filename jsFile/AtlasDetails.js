...
var downloadPath0="downloadCopy.jsp?name="+summary_re.atsID;
document.getElementById('atlas_download').href = downloadPath0;
//////////////////////celltype tsne &umap////////////////////
celltype_re="";
$.ajaxSettings.async = false;
...
//////tsne/////
	...
	var x_array=eval("cluster_re."+x).split(",");
	var y_array=eval("cluster_re."+y).split(",");
	json0["x"]=x_array;
	json0["y"]=y_array;
	json0["mode"]='markers';
	var color="cluster"+i+"_color_tsne";
	json_marker={};
	json_marker["color"]=eval("cluster_re."+color);
	json_marker["size"]=2;//点大小
	json0["marker"]=json_marker;
	data_tsen.push(json0);
	var json1={};
	...
	json1["y1"]=Math.max.apply(null,y_array);
	json1["opacity"]=0.2;
	json1["fillcolor"]=eval("cluster_re."+color);
	var json_line={};
	json_line["color"]=eval("cluster_re."+color);
	json1["line"]=json_line;
	shapes_tsen.push(json1);
//////umap/////
	var json0={};
...
tsen_umap_org(data_umap,shapes_umap,"umap_cluster","umap");
tsen_umap_org(data_tsen,shapes_tsen,"tsne_cluster","tsne");
//////////////////////cluster tsne &umap end////////////////////
/////////////////// 2.marker infor  ///////////////////
...
var data_option=advance_re.celltype_unique.split(",");
		...
		var demo1_gene = xmSelect.render({
			...
			on: function(data){//监听
				...
				//1.marker
				celltypeMarkerXMselect_2(celltype_marker,style_all_violin,ats,webpath);
	...
	//1.marker
	celltypeMarkerXMselect_2(celltype_marker_first,style_all_violin,ats,webpath);
...
if(advance_re.cellstat_enrich_table_web !="empty"){
	...
	document.getElementById("Heatmapcellstat").innerHTML="No sig result!";
}
//heatmap
...
if(advance_re.immue_enrich_table_web !="empty"){
	...
	document.getElementById("Heatmapimmue").innerHTML="No sig result!";
}
//heatmap
...
////
	...
	////
		...
		var demo1_gene = xmSelect.render({
			...
			on: function(data){//监听
				...
				var isAdd = data.isAdd;
				//
				var celltype_TF_first=eval("advance_re['"+arr[0]["value"]+"_tf"+"']").split(",");
				//1.marker
				celltypeTFXMselect(arr[0]["value"],celltype_TF_first,advance_re,ats,webpath);
				//2.tf
				//3.rbp
						}
					})
	...
	//2.tf
	celltypeTFXMselect(celltype_first,celltype_TF_first,advance_re,ats,webpath);
...
function celltypeMarkerXMselect_2(celltype_marker,style_all_violin,ats,webpath){
	var data_option=celltype_marker;
		...
		for (var i=0;i<data_option.length;i++)
					...
					var demo1_gene = xmSelect.render({
						...
						on: function(data){//监听
							...
							CopyViolinFileAndPlot(ats,webpath,style_all_violin,marker0);
							geneFeaturePlot(ats,marker0);
						}
					})
		...
		//violin
		CopyViolinFileAndPlot(ats,webpath,style_all_violin,data_option[0]);
		// feature plot////////////
		geneFeaturePlot(ats,data_option[0]);
}
//function celltypeMarkerXMselect(celltype_marker,style_all_violin,ats,webpath){
//	var data_option=celltype_marker;
//		var data_option_valuearray = [];
//		for (var i=0;i<data_option.length;i++)
//					{ 
