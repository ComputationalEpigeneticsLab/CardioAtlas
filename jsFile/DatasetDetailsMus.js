...
if (annotationType=="big"){
	document.getElementById('big_Re').setAttribute("class","btn btn-success btn-lg");
	document.getElementById('small_Re').setAttribute("class","btn btn-default btn-lg");
	//document.getElementById('big_Re').href = 'DatasetDetails.jsp?daset='+daset+'&annotationType=big';
	document.getElementById('small_Re').href = 'DatasetDetails.jsp?daset='+daset+'&annotationType=small';
}else{
	document.getElementById('small_Re').setAttribute("class","btn btn-success btn-lg");
	document.getElementById('big_Re').setAttribute("class","btn btn-default btn-lg");
	document.getElementById('small_Re').href = 'DatasetDetails.jsp?daset='+daset+'&annotationType=small';
	//document.getElementById('big_Re').href = 'DatasetDetails.jsp?daset='+daset+'&annotationType=big';
}
...
function changeAnno(type){
	if (type=="small"){
		window.location.href="DatasetDetailsMus.jsp?daset="+daset+"&annotationType=small";
	}else{
		window.location.href="DatasetDetailsMus.jsp?daset="+daset+"&annotationType=big";
		}
}

function datasetContent(daset,annotationType){
	webpath="0_files/datasets/";
	ats=daset;
	...
	var downloadPath0="downloadCopy.jsp?name="+summary_re.daset;
...
/////////////////// 2.marker infor  ///////////////////
	...
	//xmslect celltype
			...
			var demo1_gene = xmSelect.render({
				...
				on: function(data){//监听
					...
					//1.marker
					celltypeMarkerXMselect_2(celltype_marker,style_all_violin,ats,webpath,annotationType);
		...
		//1.marker
		celltypeMarkerXMselect_2(celltype_marker_first,style_all_violin,ats,webpath,annotationType);
	...
	if(advance_re.GO_BP_enrich_table_web !="empty"){
		...
		document.getElementById("HeatmapGO_BP").innerHTML="No sig result!";
	}
	//heatmap
	...
	if(advance_re.KEGG_enrich_table_web !="empty"){
		...
		document.getElementById("HeatmapKEGG").innerHTML="No sig result!";
	}
	//heatmap
...
//	}
	...
	//xmslect celltype
			...
			////
			var data_option=[];
			var keys=Object.keys(advance_re);
			for (var i=0;i<keys.length;i++){
				if(keys[i].indexOf("_tf")!=-1){
					data_option.push(keys[i].split("_tf")[0]);
				}
			}
			////
			//var data_option=advance_re.celltype_unique.split(",");
			...
			var demo1_gene = xmSelect.render({
				...
				on: function(data){//监听
					...
					var isAdd = data.isAdd;
					//
					var celltype_TF_first=advance_re[arr[0]["value"]+"_tf"].split(",");
					celltypeTFXMselect(arr[0]["value"],celltype_TF_first,advance_re,ats+"@"+annotationType,webpath);
		...
		var celltype_first=data_option[0];
			...
			//alert(celltype_first);
			
		//2.tf
		if(advance_re.tf_infor=="yes"){
			var celltype_TF_first=eval("advance_re['"+celltype_first+"_tf"+"']").split(",");
		celltypeTFXMselect(celltype_first,celltype_TF_first,advance_re,ats+"@"+annotationType,webpath);
		}else{
			document.getElementById("tf_act_content").innerHTML="No Result!";
		}
...
function celltypeMarkerXMselect_2(celltype_marker,style_all_violin,ats,webpath,annotationType){
	var data_option=celltype_marker;
		var data_option_valuearray = [];
		for (var i=0;i<data_option.length;i++)
					{ 
