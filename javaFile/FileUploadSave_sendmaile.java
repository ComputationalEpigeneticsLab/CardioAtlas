...
/**
 * Servlet implementation class FileUploadSave_sendmaile
 */
public class FileUploadSave_sendmaile extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public FileUploadSave_sendmaile() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(file_all.exists()){
			...
			for (int i = 0; i < list.length; i++){
			    ...
			    if(Math.abs(Integer.parseInt(formatDate_now)-Integer.parseInt(formatDate_file))>=60){
			    	list[i].delete();
			    }
			}
		}
