...
public class FileUploadSave extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//得到想把文件上传的真实的路径
				...
				if(file_all.exists()){
					...
					for (int i = 0; i < list.length; i++){
					    ...
					    if(Math.abs(Integer.parseInt(formatDate_now)-Integer.parseInt(formatDate_file))>=180){
					    	list[i].delete();
					    }
					}
				}
				...
				File file =new File(realPath+Randompathname); 
		...
		if (ServletFileUpload.isMultipartContent(request)){
            ...
            try {
            	//将文件写出到生成的随机文件              
                item.write(new File(realPath+Randompathname+"/"+fileName));
                String re="";
                //生成json格式的文件名和地址 ：注意在js中获取时要注意key和value的对应
                re="{" +'"'+"filename"+'"'+":"+'"'+fileName+'"'+","+'"'+"filepath"+'"'+":"+'"'+realPath+Randompathname+'"'+","+'"'+"Randompathname"+'"'+":"+'"'+Randompathname+'"'+"}";
