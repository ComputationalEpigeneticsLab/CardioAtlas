...
if(ats.contains("atlas")){
	RdataPath=RdataPath+"/CardioAtlasWebFiles/webViolin";
	orgpath=RdataPath+"/"+ats+"/violin/"+marker+"_violin.csv";
}else{
	String[] temp;
	temp = ats.split("@");
	RdataPath=RdataPath+"/CardioDatasetsWebFiles/webViolin";
	orgpath=RdataPath+"/"+temp[0]+"/"+temp[1]+"/violin/"+marker+"_violin.csv";
}


orgpath=orgpath.replaceAll("\\\\","/");
String outpath=getServletContext().getRealPath("/copyFile")+"/violin/"+marker+"_violin.csv";
