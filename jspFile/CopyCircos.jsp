...
if(ats.contains("atlas")){
	...
	orgpath=RdataPath+"/"+type+"/"+ats+"/circos/"+celltype+"_"+reg+".json";
}else{
	String[] temp;
	temp = ats.split("@");
	RdataPath=RdataPath+"/CardioDatasetsWebFiles/webReg/";
	orgpath=RdataPath+"/"+type+"/"+temp[0]+"/"+temp[1]+"/circos/"+celltype+"_"+reg+".json";
}

orgpath=orgpath.replaceAll("\\\\","/");
String outpath=getServletContext().getRealPath("/copyFile")+"/circos/"+celltype+"_"+reg+".json";
