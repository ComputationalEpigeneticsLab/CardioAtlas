cluster_sctype_annotation<-function(TaskID,filePath,innerpath,nFeature_RNA_form,nFeature_RNA_to,nCount_RNAt_form,nCount_RNA_to,percent_mt_form,percent_mt_to,
  ...
  Task_ID_new <- Task_ID
	write.table(0,paste0(filePath,"/","AAAsctypr_re.txt"),quote = F,sep = "\t",row.names = F,col.names = F,append = T)
tryCatch({
	write.table(1,paste0(filePath,"/","AAAsctypr_re.txt"),quote = F,sep = "\t",row.names = F,col.names = F,append = T)
  ######0. load base function #####
  source(paste0(innerpath,"/","function_use.R"))
  source(paste0(innerpath,"/","sctype_score.R"))
  write.table(1.2,paste0(filePath,"/","AAAsctypr_re.txt"),quote = F,sep = "\t",row.names = F,col.names = F,append = T)
  ##### read seuratobject #####
  UserSeuratObjectQC<-readRDS(paste0(filePath,"/","UserSeuratObjectQC.rds"))
   write.table(1.3,paste0(filePath,"/","AAAsctypr_re.txt"),quote = F,sep = "\t",row.names = F,col.names = F,append = T)
  #### QC del cells & genes ####
  ...
  UserSeuratObject <- NormalizeData(UserSeuratObject, normalization.method = "LogNormalize", scale.factor = 10000)
   write.table(1.38,paste0(filePath,"/","AAAsctypr_re.txt"),quote = F,sep = "\t",row.names = F,col.names = F,append = T)
  ###鎵�2000涓珮鍙樺熀鍥�
  UserSeuratObject <- FindVariableFeatures(UserSeuratObject, selection.method = "vst", nfeatures = 2000)
   write.table(1.4,paste0(filePath,"/","AAAsctypr_re.txt"),quote = F,sep = "\t",row.names = F,col.names = F,append = T)
  ###### cluster #####
  #Scaling the data
  ...
  UserSeuratObject <- RunPCA(UserSeuratObject, features = VariableFeatures(object = UserSeuratObject))
   write.table(1.5,paste0(filePath,"/","AAAsctypr_re.txt"),quote = F,sep = "\t",row.names = F,col.names = F,append = T)
  ...
  UserSeuratObject <- FindClusters(UserSeuratObject, resolution = resolution) 
   write.table(1.6,paste0(filePath,"/","AAAsctypr_re.txt"),quote = F,sep = "\t",row.names = F,col.names = F,append = T)
  #UMAP
  UserSeuratObject <- RunUMAP(UserSeuratObject, dims = 1:25, label = T)
  write.table(1.7,paste0(filePath,"/","AAAsctypr_re.txt"),quote = F,sep = "\t",row.names = F,col.names = F,append = T)
  #T-SNE
  UserSeuratObject <- RunTSNE(UserSeuratObject, dims = 1:25, label = T)
  write.table(1.8,paste0(filePath,"/","AAAsctypr_re.txt"),quote = F,sep = "\t",row.names = F,col.names = F,append = T)
  
  load(paste0(innerpath,"/",share_reference,".RData"))
  write.table(2,paste0(filePath,"/","AAAsctypr_re.txt"),quote = F,sep = "\t",row.names = F,col.names = F,append = T)
  #####run sctype 
  ...
  write.table(3,paste0(filePath,"/","AAAsctypr_re.txt"),quote = F,sep = "\t",row.names = F,col.names = F,append = T)
  #####markers
  ...
  write.table(4,paste0(filePath,"/","AAAsctypr_re.txt"),quote = F,sep = "\t",row.names = F,col.names = F,append = T)
  #####web use
  ...
  write.table(5,paste0(filePath,"/","AAAsctypr_re.txt"),quote = F,sep = "\t",row.names = F,col.names = F,append = T)
  ######3. advance out#####
  readyState<-"yes"
  out_start <- paste0("{","\n",
                      '"readyState" :','"',readyState,'",'
  ...
  color_use=color
  celltalker_re= cell_cell_conminication_italktopgenes (df_use,meta,color_use,filePath,innerpath,"user")
  ...
  write.table(6,paste0(filePath,"/","AAAsctypr_re.txt"),quote = F,sep = "\t",row.names = F,col.names = F,append = T)
  ####5. function#####
  cell_celltype_meanEXP<-funtion_infor_celltype_meanExp_mat(UserSeuratObject)
  ####5.1 enrich GO-bp heatmap&table#####
  GO_BP_re=funtion_phyper_table(marker_infor,celltype_u_noUnkown,innerpath,filePath,"GO_BP")
  ...
  ### ssgsea...
  GO_BP_heatmap_re<-funtion_infor_heatmap_go_kegg(cell_celltype_meanEXP,innerpath,filePath,'GO_BP')
  ####
  GO_GSVA<-read.delim(paste0(filePath,"/GO_BP_pathway.txt"),stringsAsFactors = F,check.names = F)
  GO_p<-read.delim(paste0(filePath,"/GO_BP_enrich_table_Download.txt"),stringsAsFactors = F,check.names = F)
  ...
  KEGG_re=funtion_phyper_table(marker_infor,celltype_u_noUnkown,innerpath,filePath,"KEGG")
  ...
  ### ssgsea...
  KEGG_heatmap_re<-funtion_infor_heatmap_go_kegg(cell_celltype_meanEXP,innerpath,filePath,'KEGG')
  ####
  KEGG_GSVA<-read.delim(paste0(filePath,"/KEGG_pathway.txt"),stringsAsFactors = F,check.names = F)
  KEGG_p<-read.delim(paste0(filePath,"/KEGG_enrich_table_Download.txt"),stringsAsFactors = F,check.names = F)
  ...
  ####5.3 enrich immue heatmap&table#####
  immue_re=funtion_phyper_table(marker_infor,celltype_u_noUnkown,innerpath,filePath,"immue")
  ...
  ### ssgsea...
  immue_heatmap_re<-funtion_infor_heatmap(cell_celltype_meanEXP,innerpath,'immue')
  ...
  ####5.4 enrich cellstat heatmap&table#####
  cellstat_re=funtion_phyper_table(marker_infor,celltype_u_noUnkown,innerpath,filePath,"cellstat")
  ...
  ### ssgsea...
  cellstat_heatmap_re<-funtion_infor_heatmap(cell_celltype_meanEXP,innerpath,'cellstat')
  ...
  write.table(8,paste0(filePath,"/","AAAsctypr_re.txt"),quote = F,sep = "\t",row.names = F,col.names = F,append = T)
  #####7.heatmap marker
